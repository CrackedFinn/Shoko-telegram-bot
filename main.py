...
keyboard = types.ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True, one_time_keyboard=True, input_field_placeholder="") # Create keyboard
...
async def get_code(message: types.Message):
    ...
    if float(sa.getBalance()['balance']) < 10:  # Get balance
        ...
        msg = str("📞 Введите данный номер для регистрации: "+ number['phoneNumber'][0:1] +"`" + number['phoneNumber'][1:] + "`\n\n" + "💰 Цена: *"+str(int(float(number['activationCost']))) + "₽*\n\n" + "🕜 Завершите регистрацию в течении *20 мин.*")
        ...
        for idfind in range(len(activations['activeActivations'])):
            if activations["activeActivations"][idfind]["activationId"] == number['activationId']:
                ...
                await bot.send_message(message.chat.id, msg, parse_mode="Markdown", reply_markup=keyboard)  # Get SMS code
                break
...
async def send_welcome(message: types.Message):
    await bot.send_message(message.chat.id, "Чтобы получить новый номер для регистрации, нажмите на кнопку ниже", parse_mode="Markdown", reply_markup=keyboard)
